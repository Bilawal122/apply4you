...
- **~~AIApply's entire logged-in product is unobserved.~~ SUPERSEDED 2026-08-04 — see section 0.4.** The original caveat read: `/jobs` is a sign-in wall; `/product` is JS-rendered and returned nothing. That was true of anonymous fetching, and several items below were written on that assumption. We have since walked their logged-in product directly. **The assumption was wrong in AIApply's favour and the affected items are corrected in 0.4.** Original text: `/jobs` is a sign-in wall; `/product` is JS-rendered and returned nothing. So there is **no verified information about their feed layout, in-product job card, whether a match score exists or how it's formatted, or their approval/review UI** — the four things we'd most want to copy or counter. The only job-card specimen anywhere on their site is one marketing mock on `/auto-apply`.
...
**0.4 CORRECTION — verified inside both products, 2026-08-04.** The founder's own logged-in AIApply session was used to walk the product. This overturns part of 0.2 and several items below. Observed directly:

- **They have a match score.** `73/100 | Good fit` on each Quick Review card. Corrects **B1**.
- **They have a genuine review UI**, and it is good: an AI **Summary** of the role, a *"Why this might be a good fit"* paragraph, **Key Matches** (green) and **Key Gaps** (red) against the JD, and **Decline / Skip / Apply** per job. Their keyword-gap feature is shipped; ours is not.
- **They run a hybrid gate, not blind fire-and-forget.** The banner reads *"Auto-applying to your best matches. Review the lower-confidence ones here."* So they auto-send high-confidence matches and route only doubtful ones to a human. Our differentiator is therefore **not** "we review and they don't" — it is "**every** application is reviewed, not just the ones the model is unsure about." **C3 gets sharper, not weaker:** an Approve-all button would collapse our position into theirs.
- **A "Rejection Reasons" page exists**: *"Auto Apply builds these patterns from jobs you've rejected… Future jobs that match will get a lower fit score, or be filtered out entirely."* This confirms **A3** is a working pattern, not a hypothesis.
- **An "Answer Library"** pre-collects answers a CV cannot contain (current salary, expected bonus, equity, GPA and its scale) with *"Answer these common application questions once and we'll fill them in automatically on future job forms."* This is our task #31, it is live for them, and it is the direct fix for our `needs_review` pile. **Promoted to a top-priority item.**
- **Credits are per-application**: "Auto Apply · 1 credit" per job, with a live balance and a Buy More Credits button.
- **Their job cards carry neither salary nor a score** — the score appears only in Quick Review, after the job is already selected.
- **Their description rendering is broken**: sentences collide with no spaces (*"how work gets done.We've built a factory…"*). We handle this correctly in `apps/web/lib/text.ts`. A small, checkable quality win to point at.

**Sprout's logged-in product remains unobserved** — no session available. Its items are still marketing-derived and should be treated as weaker evidence.
...
- **Why:** ~~AIApply has no per-job match score at all~~ — **CORRECTED 2026-08-04: they do.** Their Quick Review screen shows `73/100 | Good fit` plus a **Key Matches / Key Gaps** breakdown against the JD. It was invisible from marketing pages only. B1 is therefore no longer a category gap we are filling — it is **parity work we are behind on**, and the differentiator narrows to *decomposition we can defend* (semantic score + the named title boost + literal skill-token presence) versus their unexplained number. Sprout's score is 1–10 and only appears *after* you swipe. A visible, decomposed 0–100 shown *before* you commit is a real gap in the category.
